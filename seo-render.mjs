import { categoryPages, dietaryFor, site } from "./seo-data.mjs";
export const responsiveImages = new Map();
...
function productPrice(product) {
  const sizes = (product.sizes || []).filter(s=>s.price !== null && s.price !== "" && Number.isFinite(Number(s.price)));
  const available = sizes.filter(s=>s.status !== "sold-out");
...
function responsiveImage(image, { alt, className = "", loading = "lazy", fetchpriority = "", sizes = "(max-width: 640px) 92vw, (max-width: 1100px) 50vw, 540px" } = {}) {
  ...
  const attributes = [
    `src="${escapeHtml(/^https?:\/\//.test(cleanImage) ? cleanImage : "/" + cleanImage)}"`,
...
export { escapeHtml, absoluteUrl, productPath, categoryPage, productPage, informationPage, privacyPage, notFoundPage };
