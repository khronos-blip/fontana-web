...
- El checkout envía `expectedTotalCents`: debe ser un entero no negativo. Si difiere del total calculado, devuelve `409 pricing_changed` sin crear una reserva. El campo sigue siendo opcional para clientes anteriores. Un reintento incompatible devuelve `idempotency_conflict` e indica `activeReservation`; si la reserva anterior sigue activa, la web no genera automáticamente otra clave.
- Las cantidades iniciales solo se aplican a SKU de productos nuevos. Una opción nueva de un producto ya registrado no hereda la cantidad histórica del catálogo; comienza sin control numérico hasta que la propietaria registre sus existencias reales. Sin cambios de esquema ni migración.
